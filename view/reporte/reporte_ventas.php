...
// 2) Consulta SQL para obtener todos los detalles de ventas en un formato plano
//    Esta consulta une productos y servicios y los devuelve como una sola lista.
$query = "
    SELECT
        V.id_venta,
        V.fecha_venta,
    ...
    LEFT JOIN persona AS PER ON C.id_persona = PER.id_persona
    LEFT JOIN empresa AS EMP ON C.id_empresa = EMP.id_empresa
    INNER JOIN usuarios AS U ON V.id_usuario_vendedor = U.id_usuario
    INNER JOIN empleado AS E ON U.id_empleado = E.id_empleado
    INNER JOIN detalle_producto AS DP ON V.id_venta = DP.id_venta
    INNER JOIN producto AS P ON DP.id_producto = P.id_producto
    WHERE
         V.estado = 1 AND DP.estado = 1
    ...
    INNER JOIN usuarios AS U ON V.id_usuario_vendedor = U.id_usuario
    INNER JOIN empleado AS E ON U.id_empleado = E.id_empleado
    INNER JOIN detalle_servicio AS DS ON V.id_venta = DS.id_venta
    INNER JOIN servicios AS S ON DS.id_servicio = S.id_servicio
    WHERE V.estado = 1 AND DS.estado = 1

    ORDER BY id_venta ASC, tipo_item ASC;
";
        ...
        <table class="header-table">
          <tr>
            ...
            <td>
              ...
              <div class="small">Detalle de Productos y Servicios Vendidos</div>
            </td>
          </tr>
        </table>
...
if ($rows) {
    // 4) Se recorren todos los resultados y se crea una fila <tr> por cada uno
    foreach ($rows as $r) {
        $html .= '<tr>'
    ...
    // Se ajusta el colspan al nuevo número de columnas (9)
    $html .= '<tr><td colspan="9" class="small">No hay ventas para mostrar.</td></tr>';
}
